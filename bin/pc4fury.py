...
def get_config_files():
    ...
    path_to_share = os.path.join('~', '.local','share','pc4fury')
    dirname = os.path.expanduser(path_to_share)
    try:
        os.makedirs(dirname)
    except:
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/pc4fury")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "pc4fury")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB pc4fury' -t 'Your Run completed.'&")
...
#    animate_tab.gen_button.disabled = True
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 pc4fury-r7 config.xml")   # "-r7" suffix??
        else:
...
tool_title = widgets.Label(r'\(\textbf{PhysiCell-Fury 3D Demo}\)')
if nanoHUB_flag or hublib_flag:
